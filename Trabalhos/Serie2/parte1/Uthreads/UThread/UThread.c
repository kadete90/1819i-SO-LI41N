...
//b) Mantenha uma lista de todas as threads em actividade.
static
...
VOID UtExit () {
	...
	//RunningThread->AliveLink.Flink = NULL; RunningThread->AliveLink.Blink = NULL;
	//vs
...
//
VOID UtDeactivate() {
	((PUTHREAD)RunningThread)->State = BLOCKED;
	Schedule();
}
...
//a) Adicione à API a função INT UtThreadState(HANDLE thread) que retorna o estado da thread passada por parâmetro. 
//	 Faça as alterações necessárias para manter o estado actualizado.
INT UtThreadState(HANDLE handle)
{
	return ((PUTHREAD)handle)->State;
}

//b) Realize a função BOOL UtAlive(HANDLE thread) que retorna true se o handle passado como argumento corresponder ao de uma thread em actividade. 
//	 Entende-se por thread em actividade qualquer thread que tenha sido criada e ainda não tenha terminado (não tenha invocado a função UtExit), 
//	 independentemente do seu estado.

BOOL UtAlive(HANDLE handle)
{
	LIST_ENTRY *curr = AliveThreads.Flink;
	PUTHREAD thread = (PUTHREAD)handle;
	while (curr != &AliveThreads) {
		if (&thread->AliveLink == curr) return TRUE;
		curr = curr->Flink;
	}
	return FALSE;
}

//c) Realize a função DWORD UtJoin(HANDLE thread) que não retorna enquanto não terminar a thread correspondente ao handle passado como argumento 
//	 ou a operação Join não tiver sido cancelada. A função retorna:
//		1 com a terminação da thread thread,
//		0 se o handle passado não corresponder a uma thread em actividade, 
//		-1 se a operação tiver sido cancelada. 
// Considere uma thread em actividade, qualquer thread que ainda não tenha evocado a função UtExit.

DWORD UtJoin(HANDLE handle) 
{
	PUTHREAD thread = (PUTHREAD)handle;
	if (thread == UtSelf() || !UtAlive(thread))
	{
		return 0;
	}

	if (thread->JoinCanceled)
	{
		return -1;
	}
	
	RunningThread->State = WAIT_JOIN;
	RunningThread->JoinCanceled = FALSE;

	RemoveEntryList(&thread->Link);
	InsertTailList(&ReadyQueue, &thread->Link); // insert at tail or head?
		
	Schedule();

	return RunningThread->JoinCanceled ? -1 : 1;
}

//d) Realize a função BOOL UtJoinCancel(HANDLE thread) que cancela a operação Join executada pela thread correspondente ao handle passado como argumento.
//	 O sucesso da operação depende do handle passado corresponder a uma thread bloqueada na operação Join.

BOOL UtJoinCancel(HANDLE handle) 
{	
	if (handle == UtSelf() || !UtAlive(handle) || UtThreadState(handle) != WAIT_JOIN)
	{
		return FALSE;
	}

	PUTHREAD thread = (PUTHREAD)handle;
	thread->JoinCanceled = TRUE;
	RemoveEntryList(&thread->Link);
	UtActivate(thread);
	return TRUE;
}

//e) Acrescente a função VOID UtSwitchTo(HANDLE threadToRun), que provoca uma comutação imediata de contexto para a thread threadToRun, 
//	 se esta se encontrar no estado ready. Se não for esse o caso a função não tem nenhum efeito.

VOID UtSwitchTo(HANDLE handle) 
{
	if (handle == UtSelf() || !UtAlive(handle) || UtThreadState(handle) != READY)
	{
		return;
	}

	PUTHREAD threadToRun = (PUTHREAD)handle;
	RemoveEntryList(&threadToRun->Link);
	InsertHeadList(&ReadyQueue, &RunningThread->Link);
